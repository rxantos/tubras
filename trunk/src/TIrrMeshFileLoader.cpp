...
namespace Tubras
{
    ...
    //! Constructor
    TIrrMeshFileLoader::TIrrMeshFileLoader(scene::ISceneManager* smgr,
        io::IFileSystem* fs)
    ...
    bool TIrrMeshFileLoader::isALoadableFileExtension(const c8* fileName) const
    {
    ...
    IAnimatedMesh* TIrrMeshFileLoader::createMesh(io::IReadFile* file)
    {
    ...
    IAnimatedMesh* TIrrMeshFileLoader::readMesh(io::IXMLReader* reader)
    {
    ...
    IMeshBuffer* TIrrMeshFileLoader::readMeshBuffer(io::IXMLReader* reader)
    {
    ...
    //! read indices
    void TIrrMeshFileLoader::readIndices(io::IXMLReader* reader, int indexCount, core::array<u16>& indices)
    {
    ...
    void TIrrMeshFileLoader::readMeshBuffer(io::IXMLReader* reader, int vertexCount, SMeshBuffer* sbuffer)
    {
        core::stringc data = reader->getNodeData();
        const c8* p = &data[0];
    ...
    void TIrrMeshFileLoader::readMeshBuffer(io::IXMLReader* reader, int vertexCount, SMeshBufferLightMap* sbuffer)
    {
        core::stringc data = reader->getNodeData();
        const c8* p = &data[0];
    ...
    void TIrrMeshFileLoader::readMeshBuffer(io::IXMLReader* reader, int vertexCount, SMeshBufferTangents* sbuffer)
    {
        core::stringc data = reader->getNodeData();
        const c8* p = &data[0];
    ...
    void TIrrMeshFileLoader::skipSection(io::IXMLReader* reader, bool reportSkipping)
    {
#ifdef _DEBUG
    ...
    inline f32 TIrrMeshFileLoader::readFloat(const c8** p)
    {
        f32 ftmp;
    ...
    inline s32 TIrrMeshFileLoader::readInt(const c8** p)
    {
    ...
    void TIrrMeshFileLoader::skipCurrentNoneWhiteSpace(const c8** start)
    {
        const c8* p = *start;
    ...
    //! places pointer to next begin of a token
    void TIrrMeshFileLoader::findNextNoneWhiteSpace(const c8** start)
    {
        const c8* p = *start;
    ...
    void TIrrMeshFileLoader::readFloatsInsideElement(io::IXMLReader* reader, f32* floats, u32 count)
    {
        if (reader->isEmptyElement())
            return;
