# --------------------------------------------------------------------------
# Lightmap Pack UV Projection Unwrapper v1.2 by Campbell Barton (AKA Ideasman)
# --------------------------------------------------------------------------
...
class LightmapUVPack(bpy.types.Operator):
    ...
    active_only = BoolProperty(name='Active Object',
                               description='If disabled, include other selected objects for packing the lightmap.',
                               default=True)

    selected_faces = BoolProperty(name='Selected Faces',
                                  description='Use only selected faces from all selected meshes.',
                                  default=True)

    share_tex_space = BoolProperty(name='Share Tex Space',
                                   description='Objects Share texture space, map all objects into 1 uvmap.',
                                   default=False)

    new_uv_layer = BoolProperty(name='New UV Layer',
                                description='Create a new UV layer for every mesh packed.',
                                default=False)

    new_image = BoolProperty(name='New Image',
                             description='Assign new images for every mesh (only one if shared tex space enabled)',
                             default=False)

    image_size = IntProperty(name='Image Size',
                             description='Width and Height for the new image',
                             default=512, min=64, max=5000)

    pack_quality = IntProperty(name='Pack Quality',
                               description='Pre Packing before the complex boxpack',
                               default=12, min=1, max=48)

    margin_size = FloatProperty(name='Margin',
